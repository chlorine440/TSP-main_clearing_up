...
extern float servo1_x ;
extern float servo2_y;
...
void tsp_servo_control_pid(float target_x, float target_y, float current_x, float current_y){
    ...
    float output_x = tsp_pid_control(target_x, current_x, kp_servo, ki_servo, kd_servo);
    float output_y = tsp_pid_control(target_y, current_y, kp_servo, ki_servo, kd_servo);
    // if(output_x > 10) output_x = 10; // 限制最大偏移
    // if(output_x < -10) output_x = -10; // 限制最小偏移
    // if(output_y > 10) output_y = 10; // 限制最大偏移
    // if(output_y < -10) output_y = -10; // 限制最小偏移
    // 控制舵机
    servo1_x -= output_x;
    servo2_y -= output_y;
